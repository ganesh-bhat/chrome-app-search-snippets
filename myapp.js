...
initialize = function() {
	var snippets = {
		snippetUnderEditId:0,
		filter:"ALL",
		delSticky:new Object(),
		sortType:1,
		
		init: function(){
			this.cacheDom();
			this.bindEvents();
			this.prepareData(this.render.bind(this));
		},
		cacheDom:function(){
			/* nav bar dom elements */
			this.$navBar = $("#navBar");
			this.$searchBar = $('#searchBar');
			this.$showFavoritesBtn = $('#showFavoritesBtn');
			this.$sortTitleRadio = $('#sortTitle');
			this.$serarchInput = $('#serarchSnippetField');
			this.$serarchBtn = $('#searchGoBtn');
			this.$resultFilterRadio = $('#filterBtns');
			this.$dropDownMenu = $('#dropDownMenu');
			this.$exportToJsonBtn=$('#exportAllJsonBtn');
			this.$exportToClearTxtBtn=$('#exportAllClearTextBtn');
			
			/* snippet scratch panel*/
			this.$snippetScratchPanel = $("#popOutModal");
			
			this.$scratchPanelContent = $('#modelContent');
			this.$scratchPanelTitle = $('#scratchPanelTitle');
			
			
			/* snippet display panel*/
			this.$snippetDisplayPanel = $("#snippetDisplayPanel");
			this.$snippetTagsList = $('#tagsList');
			this.$snippetAppMessages =$('#appMessages');
			this.$snippetsList = $('#snippetLists');
			
			
			/* snippet creator panel*/
			this.$snippetCreatorPanel = $('#newSnippet');
			this.$isFavoriteChkbox = $('#isFavoriteChkbox');
			this.$snippetTitle = $('#snippetTitle');
			this.$snippetTags = $('#snippetTags');
			this.$snippetContent = $('#snippetContent');
			
			this.$snippetCreateBtn = $('#createSnippetBtn');
			this.$snippetUpdateBtn = $('#updateSnippetBtn');
			this.$snippetCancelBtn = $('#cancelSnippetCreateBtn');
		
		},
		bindEvents:function(){
			console.log("bind events");
			/* snippet creator panel events*/
			this.$snippetCreateBtn.on('click',this.createSnippetBtn.bind(this));
			this.$snippetUpdateBtn.on('click',this.updateSnippet.bind(this));
			this.$snippetCancelBtn.on('click',this.cancelSnippetBtn.bind(this));
			
			/* nav bar events */ 
			this.$showFavoritesBtn.on('click',this.showFavouriteSnippets.bind(this));
			this.$serarchBtn.on('click',this.searchAsType.bind(this));
			
			this.$serarchInput.on("input", this.searchAsType.bind(this));
			
		},
		prepareData:function(onResultCallback) {
			console.log("prepareData");
			snippetsDB.open(onResultCallback);
		},
		...
		createSnippetBtn : function(event){
			var newSnippet = new Object();
			console.log("Create snippet called");

			newSnippet.title = $("#snippetTitle").val();
			console.log("title is:"+newSnippet.title);
		...
		backToListing : function(){
			console.log("back to listing called");
			$("#snippetLists").empty();
...
init = function(){
	...
	/* sort functions */
	$('input:radio[name="filterTag"]').change(onFilterChange);
